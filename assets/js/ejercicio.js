...
button.onclick = function() {
    ...
    if (opc>=1 && opc<=(canciones.length)){ 
        ...
        const audio = new Audio(pathFile[opc]);
            ...
            const musica =  (opc-1) == 0 ? `Reproduciendo ${opc} ${audio.play()}`:
                            (opc-1) == 1 ? `Reproduciendo ${opc} ${audio.play()}`:
                            (opc-1) == 2 ? `Reproduciendo ${opc} ${audio.play()}`:
                            (opc-1) == 3 ? `Reproduciendo ${opc} ${audio.play()}`:
                            (opc-1) == 4 ? `Reproduciendo ${opc} ${audio.play()}`:
                            (opc-1) == 5 ? `Reproduciendo ${opc} ${audio.play()}`:'No existe la cancion'; //caso contratrio =>F
...
// let crearSong = window.confirm('Quieres adicionar una cancion?');
// if (crearSong === true) {
